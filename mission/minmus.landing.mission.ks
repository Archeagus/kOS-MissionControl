...
local ver is "ver. APEX-MMP-0.0.9".
...
local mission is mission_control({ parameter seq, ev, next.
	...
	seq:add({
		action("EVA Crew Recovered"). countdown(5).
		lock steering to heading(90, 90).
		lock throttle to 1.
		wait 2. gear off.
	...
	seq:add({
		if not (status = "Waiting for Reentry Burn") action("Waiting for Reentry Burn").
