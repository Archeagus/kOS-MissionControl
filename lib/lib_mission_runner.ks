{
	function mission_runner {
		...
		until done {
			sequence[runmode*2+1](update).
			
			for event in events:values event(update).
			wait 0.01.
		}
		
		if core:volume:exists("mission.runmode") core:volume:delete("mission.runmode").
		
		function update_runmode {
			parameter n.
			
			if not core:volume:exists("mission.runmode") core:volume:create("mission.runmode").
